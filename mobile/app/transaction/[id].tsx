...
export default function EditTransactionScreen() {
    ...
    async function loadData() {
        try {
            ...
            const transaction = transRes.data.find((t: any) => t.id === id);
            if (transaction) {
    ...
    async function handleSave() {
        ...
        try {
            ...
            Alert.alert('Erro', error.response?.data?.message || 'Erro ao atualizar transação.');
        } finally {
            setLoading(false);
        }
    }
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
                ...
                <View style={styles.categoriesGrid}>
                    {filteredCategories.map((cat) => (
                        <TouchableOpacity
                            key={cat.id}
                            style={[styles.categoryItem, selectedCategory === cat.id && { borderColor: cat.color, borderWidth: 2 }]}
...
const styles = StyleSheet.create({
    ...
    categoriesGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 32 },
    categoryItem: {
        width: '30%',
        alignItems: 'center',
        backgroundColor: Colors.surface,
    ...
    categoryIcon: { width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
    categoryName: { fontSize: 11, color: Colors.text, textAlign: 'center' },
    saveBtn: { backgroundColor: Colors.primary, height: 54, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
